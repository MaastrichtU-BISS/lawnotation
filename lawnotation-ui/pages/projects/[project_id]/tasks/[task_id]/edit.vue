    ...
    <div class="dimmer-content">
      <div class="grid md:grid-cols-2 md:gap-6">
        ...
        <section class="flex flex-col gap-6">
          <Card v-if="new_emails.length" class="flex flex-col gap-4 h-fit">
            <template #title>
              <h3 class="text-lg font-semibold">Replace Annotators</h3>
            </template>
            <template #content>
              <ul>
                <li v-for="(email, index) in new_emails" :key="`${annotators[index]?.id ?? index}`">
          ...
          <form>
            <Fieldset
              ...
              :pt="{
                ...
                legend: { class: '!text-red-500 !bg-inherit border-none' },
              }"
...
const editTask = async () => {
  ...
  if (new_task.value.ann_guidelines) {
    try {
      new URL(new_task.value.ann_guidelines);
    } catch {
...
const replaceAnnotators = async () => {
  ...
  try {
    ...
    annotators.splice(0, annotators.length, ...update.annotators);
...
onMounted(async () => {
  ...
  project.value = await $trpc.project.findById.query(projectId);
  ...
  // Ensure the task's labelset is in the list
  if (currentTask.labelset_id) {
    labelset.value = await $trpc.labelset.findById.query(
      currentTask.labelset_id,
    );
    if (
      labelset.value &&
      !labelsets.value.find((l) => l.id === currentTask.labelset_id)
    ) {
      labelsets.value.push(labelset.value);
    }
  }

  numberOfAssignments.value = await $trpc.assignment.getCountByTask.query(
    currentTask.id,
  );
  ...
  new_task.value = {
    ...
    ann_guidelines: currentTask.ann_guidelines,
    project_id: currentTask.project_id,
    labelset_id: currentTask.labelset_id,
    ml_model_id: currentTask.ml_model_id,
    origin_task_1_id: currentTask.origin_task_1_id,
    origin_task_2_id: currentTask.origin_task_2_id,
  };

  annotators.splice(0) &&
    annotators.push(
      ...(await $trpc.task.getAllAnnotatorsFromTask.query(taskId)),
    );
...
definePageMeta({
  middleware: [
    "auth",
    async (to) => authorizeClient([["task", Number(to.params.task_id)]]),
    async (to) => authorizeClient([["project", Number(to.params.project_id)]]),
  ],
});
</script>