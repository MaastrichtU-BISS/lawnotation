  ...
  <div v-if="task" class="dimmer-wrapper pt-2">
    ...
    <div class="dimmer-content">
      <div class="justify-center">
        <div
          class="flex flex-col w-1/2 mx-auto mt-8 space-y-6 border-neutral-300"
        >
        ...
        <div
          v-if="new_emails && new_emails.length"
...
const editTask = async () => {
  ...
  try {
    const updates: any = {};
    if (new_task.value.name !== task.value?.name) {
      updates.name = new_task.value.name;
    }
    if (new_task.value.desc !== task.value?.desc) {
      updates.desc = new_task.value.desc;
    }
    if (new_task.value.ann_guidelines !== task.value?.ann_guidelines) {
      updates.ann_guidelines = new_task.value.ann_guidelines;
    }
    if (new_task.value.labelset_id !== task.value?.labelset_id) {
      updates.labelset_id = new_task.value.labelset_id;
    }
...
onMounted(async () => {
  ...
  if (task.value.labelset_id) {
    labelset.value = await $trpc.labelset.findById.query(
      task.value.labelset_id,
    );
    if (
      labelset.value &&
      !labelsets.value.find((l) => l.id === task.value.labelset_id)
    ) {
  ...
  new_task.value = {
    ...
    ml_model_id: task.value.ml_model_id,
    origin_task_1_id: task.value.origin_task_1_id,
    origin_task_2_id: task.value.origin_task_2_id,
  };

  annotators.splice(0) &&
    annotators.push(
      ...(await $trpc.task.getAllAnnotatorsFromTask.query(
