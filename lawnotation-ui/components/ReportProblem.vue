...
<template>
    <ConfirmPopup>
        <template #container="{ message }">
            <div class="rounded-lg drop-shadow-2xl shadow-inner p-6">
                <button @click="closePopup()" class="absolute top-2 right-2 text-gray-800 hover:text-gray-600 focus:outline-none">
    ...
    <Button @click="openPopup($event)" label="Report problem" outlined class="bg-white !fixed bottom-2 right-2 z-50 w-24 h-10 text-xs font-light text-primary-500 border border-primary-500 hover:scale-110 hover:bg-white">
        <template #icon>
