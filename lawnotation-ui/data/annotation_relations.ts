...
export const useAnnotationRelationApi = () => {
  ...
  const createRelation = async (
    fields: LSSerializedRelation,
    from_id: number,
    to_id: number
  ): Promise<AnnotationRelation> => {
    const { data, error } = await supabase
      .from("annotation_relations")
      .insert(convert_ls2db(fields, from_id, to_id) as never)
      .select()
      .single();

    if (error) throw Error(`Error in createRelation: ${error.message}`);
    else return data as AnnotationRelation;
  };

  const createRelations = async (
    fields: Omit<AnnotationRelation, "id">[]
  ): Promise<AnnotationRelation[]> => {
    const { data, error } = await supabase
      .from("annotation_relations")
      .insert(fields)
    ...
    if (error) throw Error(`Error in createRelations: ${error.message}`);
    else return data as AnnotationRelation[];
  };
  ...
  const findRelation = async (id: string): Promise<AnnotationRelation> => {
    ...
    if (error) throw Error(`Error in findRelation: ${error.message}`);
    else return data as AnnotationRelation;
  };
