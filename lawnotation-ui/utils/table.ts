export type TableData<T> = {
  ...
  sort: {
    column:
      | null
      | TableData<T>["columns"][keyof TableData<T>["columns"]]["field"];
    dir: "ASC" | "DESC";
  };
  search: {
    column:
...
export const createTableData = <T>(
  columns: TableData<unknown>["columns"],
  src_options: CreateTableDataSource,
  default_sort?: TableData<T>["sort"]
): TableData<T> => {
  ...
  const td: TableData<T> = reactive({
    ...
    load: async () => {
      ...
      switch (src_options.type) {
        case "table": {
          const filter: object =
            (typeof src_options.filter === "function"
              ? src_options.filter()
              : src_options.filter) ?? {};
          ...
          if (error)
            throw Error(
              `Error in tablequery from ${src_options.from}: ${error.message}`
            );
